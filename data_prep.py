...
def extract_party(p):
    # EXTRACT TEXT BETWEEN PARENTHESIS, WHERE PARTY INFORMATION IS
    # EXAMPLE: av Robert Stenkvist (SD)
    start = p.find("(") + len("(")
    end = p.find(")")
...
def extract_data(fn):
    # EXTRACT METADATA (DATE AND PARTY) AND TEXT OF MOTION FROM HTML FORMAT
    # THIS OPTION IS USED FOR NEWER DATA (AFTER 2010), AS THAT HAS
    ...
    # EXTRACT TEXT FROM ALL PARAGRAPH TAGS UNLESS NAMES OF SIGNEES
    ...
    try:   
        ...
        if ds == None:
            ds = document.find('div', 'sidhuvud_beteckning')
        date = ds.string
        date = date[0:7]
    except: date = None
    ...
    try:
        ...
        if ws == None:
            ws = document.find('div','MotionarLista')
...
def main():
    ...
    for k, v in meta_data.items():
        ...
        # SPLIT BY SESSION AND CHECK IF WITHIN STUDIED PERIOD
