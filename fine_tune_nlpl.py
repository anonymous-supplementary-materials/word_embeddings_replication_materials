...
def main():
    ...
    for iter_num in range(10):
        ...
        # INITIALIZE MODEL
        params = dict(min_count=1, workers=1, iter=args.epochs, size=100, seed=1)
        ...
        training_examples_a = sklearn.utils.resample(training_examples, replace=True, n_samples=len(training_examples), random_state=iter_num)

        # TRAIN MODEL
        model.train(training_examples_a, total_examples=len(training_examples_a),
                    epochs=model.epochs, compute_loss=True, callbacks=[callback()])
        
        # EXTRACT AND SAVE LEARNED EMBEDDINGS
        res = model.wv
        if args.output_folder:
            out = args.output_folder
        else:
            out = ''
        t_span = args.time_span.replace('-', '_')
        res.save(out + "nlpl_" + t_span + "_" + args.party + "_" + str(iter_num) + '.kv')
