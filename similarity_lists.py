...
def aggregate_list(models, term, topn, vocab):
    #GO THROUGH ALL KeyedVectors AVAILABLE AND CALCULATE 
    #COSINE SIMILARITY AND STD TO ALL TERMS IN THE VOCABULARY 
    #FOR THE SELECTED TERM OF INTEREST GIVEN AS INPUT TO THIS FUNCTION
    #RETURN THE TOP topn CLOSEST TERMS
    #VACABULARY CAN BE GIVEN TO IGNORE TERMS NOT IN THE FINE-TUNING DATA
...
def print_to_file(time_span, party, pre_train, model_folder):
    #FOR A GIVEN TIME SPAN, PARTY AND TYPE OF PRE-TRAINING
    #EXTRACT TOP 20 MOST SIMILAR TERMS TO THE LIST OF STUDIED 
    #TERMS DEFINED BELOW
    #WRITE TERM, COSINE SIMILARITY SCORE, STD, TERM FREQ AND DOCUMENT FREQ TO FILE 
    ...
    if time_span in ['1988-2009', '1988-2020']:
        with open('processed_data/data_1988_2009.pkl', 'rb') as f:
            data_dict.update(pickle.load(f))

    if time_span in ['1988-2020', '2010-2020']:
        with open('processed_data/data_2010_2020.pkl', 'rb') as f:
            data_dict.update(pickle.load(f))
